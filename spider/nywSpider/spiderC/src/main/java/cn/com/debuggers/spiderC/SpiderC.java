package cn.com.debuggers.spiderC;
...
public class SpiderC {


    public static void main(String[] args) {
        SpiderC spider = new SpiderC();
        spider.start();
    }
    ...
    public void start() {
        try {
            ...
            for (int i = 0; i < CompanyList.size(); i++) {
                ...
                String income = "null", profit = "null", netProfit = "null", totalAssets = "null", totalLiability = "null", netAssets = "null";
                try {
                    JSONObject finance = jsonObj.getJSONObject("finance");
                    income = finance.getString("income").replace("\"", "");
                    profit = finance.getString("profit").replace("\"", "");
                    netProfit = finance.getString("netProfit").replace("\"", "");
                    totalAssets = finance.getString("totalAssets").replace("\"", "");
                    totalLiability = finance.getString("totalLiability").replace("\"", "");
                    netAssets = finance.getString("netAssets").replace("\"", "");
...
//                        netProfit + "\ttotalAssets:" + totalAssets + "\ttotalLiability:" + totalLiability + "\tnetAssets:" + netAssets);
                ...
                try {
                    db.insertFinance(companyName, income, profit, netProfit, totalAssets, totalLiability, netAssets);
                } catch (Exception e) {
                    System.out.println(i + "\t" + code + "\t" + companyName + "\tincome:" + income + "\tprofit:" + profit + "\tnetProfit:" +
                            netProfit + "\ttotalAssets:" + totalAssets + "\ttotalLiability:" + totalLiability + "\tnetAssets:" + netAssets);
                    e.printStackTrace();
            ...
            System.out.println("Finish spiderC.");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
