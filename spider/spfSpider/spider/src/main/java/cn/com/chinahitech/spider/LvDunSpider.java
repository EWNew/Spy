...
//        System.out.println(next_url);
    ...
    private void parseHtmlRange3(String content) {
        try{
            ...
            Elements elements=document.select("div.comTable");
//            String tongYiXinYong=elements.select("tr").last().select("td").html();
            String zhuCeZiBen=elements.select("tr").get(4).attr("td")+elements.select("tr").get(4).select("span.comshow_16").html();
//            String chengLiRiQi = elements.select("tr").get(5).select("td").html().substring(0, 10);
//            String gongSiLeiXing=elements.select("tr").get(2).select("span.comshow_16").html().replaceAll("\n","");
//            String yingYeQiXian=elements.select("tr").get(7).select("td").html();
 //           DBManager db=new DBManager();
 //           db.addSpfInfo(this.companyName,tongYiXinYong,chengLiRiQi,gongSiLeiXing,yingYeQiXian);
    ...
    private String parseHtmlRange2(String content) {
        Document document=Jsoup.parse(content);
        String next_Url=document.select("div[class=cont_lcen cont_l_v1]>div.v1-business>ul>li").first().select("a").attr("href");
        return next_Url;
    }
...
//                LvDunSpider lvDunSpider = new LvDunSpider();
//                lvDunSpider.setName(companyList.get(i));
//                lvDunSpider.requestData(companyList.get(i));
//                try{
...
//        }
        LvDunSpider lvDunSpider=new LvDunSpider();
        lvDunSpider.requestData("北京时代科技股份有限公司");
    }
}
