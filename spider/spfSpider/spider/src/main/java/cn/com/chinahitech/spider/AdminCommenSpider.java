...
public class AdminCommenSpider {
...
//        System.out.println(next_url);
    ...
    private String parseHtmlRange2(String content) {
        Document document=Jsoup.parse(content);
        String next_Url=document.select("div[class=cont_lcen cont_l_v1]").select("h4[class=v1_title ]").get(3).select("a").attr("href");
        return next_Url;
    }
    ...
    private void parseHtmlRange3(String content) {
        try {
            ...
            for(int i=1;i<elements.size();i++){
                dbManager.addAdmin(companyName,elements.get(i).select("a").html(),"https://www.11315.com"+elements.get(i).select("a").attr("href"),elements.get(i).select("td").get(1).attr("title"),elements.get(i).select("td").last().html());
...
//            System.out.println(RiQi);
    ...
    public static void main(String[] args){
        try {
            ...
            for(int i=6881;i<companyList.size();i++) {
                System.out.println("第"+i+"家公司: "+companyList.get(i));
                AdminCommenSpider adminCommenSpider = new AdminCommenSpider();
                adminCommenSpider.setName(companyList.get(i));
                adminCommenSpider.requestData(companyList.get(i));
                try{
