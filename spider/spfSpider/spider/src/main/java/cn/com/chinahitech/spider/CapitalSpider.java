...
//        System.out.println(url);
        ...
        try {
            ...
            // 7. 数据抽取
            String capital = parseHtmlRange(content);
            Pattern pattern=Pattern.compile("([1-9]\\d*\\.?\\d*)|(0\\.\\d*[1-9])");
            Matcher matcher=pattern.matcher(capital);
            capital=matcher.find()?matcher.group(1):"0";

            DBManager dbManager=new DBManager();
            dbManager.addCapital(companyName,capital);
    ...
    private String parseHtmlRange(String content) {
        ...
        String next_Url = document.select("div[id=main]>div.content").select("div[class=search_company clearfix]").first().select("ul").first().select("span").get(1).html();
        return next_Url;
    }
...
//        Document document=Jsoup.parse(content);
//        String capital=document.select("div.v1Table").select("tbody").select("tr").get(1).select("td").get(1).html();
//        Pattern pattern=Pattern.compile("([1-9]\\d*\\.?\\d*)|(0\\.\\d*[1-9])");
//        Matcher matcher=pattern.matcher(capital);
//        capital=matcher.find()?matcher.group(1):"0";
//        DBManager dbManager=new DBManager();
...
//    }
    ...
    public static void main(String[] args){
        ...
        try {
            ...
            for(int i=3345;i<companyList.size();i++) {
                System.out.println("第"+i+"家公司: "+companyList.get(i));
                CapitalSpider capitalSpider =new CapitalSpider();
                capitalSpider.setName(companyList.get(i));
                capitalSpider.requestData(companyList.get(i));
                try{
