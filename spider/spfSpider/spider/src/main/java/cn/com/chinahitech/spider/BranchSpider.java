...
//        System.out.println(next_url);
    ...
    private String parseHtmlRange2(String content) {
        Document document=Jsoup.parse(content);
        String next_Url=document.select("div[class=cont_lcen cont_l_v1]>div.v1-business>ul>li").get(4).select("a").attr("href");
        return next_Url;
    }
...
//        System.out.println(next_url);
    ...
    private void parseHtmlRange3(String content) {
        try {
            ...
            Document document=Jsoup.parse(content);
            Elements elements=document.select("div.comTable").select("tbody").select("tr");
            DBManager dbManager=new DBManager();
            for(int i=1;i<elements.size();i++){
                dbManager.addBranch(companyName,elements.get(i).select("a").html(),"https://www.11315.com"+elements.get(i).select("a").attr("href"),elements.get(i).select("td").get(2).html());
//                Namelist.add(elements.get(i).select("a").html());
//                NameLink.add("https://www.11315.com"+elements.get(i).select("a").attr("href"));
//                DanWei.add(elements.get(i).select("td").get(2).html());
...
//            System.out.println(RiQi);
    ...
    public static void main(String[] args){
        try {
            ...
            for(int i=1303;i<companyList.size();i++) {
                System.out.println("第"+i+"家公司: "+companyList.get(i));
                BranchSpider branchSpider=new BranchSpider();
                branchSpider.setName(companyList.get(i));
                branchSpider.requestData(companyList.get(i));
                try{
